...
int countSteps(map<string, deque<float>> data);
int countStepsWithDataOutput(map<string, deque<float>> data);
map<string, deque<float>> getData(string filePath);
...
bool isPeak(float localData[], int interval);
int findPeakInterval(float localData[], int startInterval, int endInterval);
int findPeak(float localData[], int i);
						...
						for (int stepBufferBuffer : stepBufferInterval) {
							for (int peakAverageBuffer : peakAverageInterval) {
								...
								for (auto itr = filePathMap.begin(); itr != filePathMap.end(); itr++) {
									...
									// Get x, y, z accelerometer data for easy access
									// Pretend vector is the address of the accelerometer
									map<string, deque<float>> data = getData(itr->first);

									int steps = countSteps(data);
		...
		if (delay == 0) {
			if (!walking) {
				...
				if (localData[i] > THRESHOLD || stepBuffer > 0) {
					...
					int peakInterval = findPeak(localData, i);
...
int countStepsWithDataOutput(map<string, deque<float>> data) {
    ...
    while (true) {
		float x = readByte(data, "x");
		float y = readByte(data, "y");
		float z = readByte(data, "z");
        // Imitate reading from the accelerometer address
        // Store the last "MOVE_MEAN_SIZE" elements
        //moveMeanData[i % MOVE_MEAN_SIZE] = pow(readByte(data, "x"), 2) + pow(readByte(data, "y"), 2) + pow(readByte(data, "z"), 2);
...
map<string, deque<float>> getData(string filePath) {
	map<string, deque<float>> vectorMap; // Map of x, y, z accelerometer data
	...
	while (in.good())
	{
		...
		if (subParts.size() >= 4) {
            vectorMap["x"].push_back(atof(subParts[1].c_str()));
            vectorMap["y"].push_back(atof(subParts[2].c_str()));
            vectorMap["z"].push_back(atof(subParts[3].c_str()));
		}
	}
...
bool isPeak(float localData[], int interval) {
	return localData[interval] > THRESHOLD && 
		localData[interval] > average(localData, MEMORY_SIZE, (interval - PEAK_AVERAGE) % MEMORY_SIZE, interval) &&
		localData[interval] > average(localData, MEMORY_SIZE, interval, (interval + PEAK_AVERAGE) % MEMORY_SIZE);
}

int findPeakInterval(float localData[], int startInterval, int endInterval) {
	int peakInterval = -1;
	float peakValue = 0.0;
	for (int i = startInterval; i != endInterval; i = (i + 1) % MEMORY_SIZE) {
		if (isPeak(localData, i) && localData[i] > peakValue) {
...
int findPeak(float localData[], int i) {
	return findPeakInterval(localData, (i + (((int) (SAMPLE_RATE / MAX_HERTZ)))) % MEMORY_SIZE, (i + ((int) (SAMPLE_RATE / MIN_HERTZ))) % MEMORY_SIZE);
}
